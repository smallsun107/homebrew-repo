class Godzillaekp < Formula
    ...
    # end
    def install
        libexec.install "Godzilla_ekp1.1.jar"
        (bin/"godzilla_ekp").write <<~EOS
            #!/bin/bash
            mkdir -p "$HOME/.config/godzilla_ekp"
            cd "$HOME/.config/godzilla_ekp"
            exec java -jar "#{libexec}/Godzilla_ekp1.1.jar" "$@"
        EOS
        chmod 0755, bin/"godzilla_ekp"
    end
end